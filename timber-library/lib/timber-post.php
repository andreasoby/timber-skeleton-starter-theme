...
class TimberPost extends TimberCore implements TimberCoreInterface {
    ...
    function get_content($len = 0, $page = 0) {
        ...
        $postcustom = get_post_custom_keys($this->ID);
        if (count($postcustom) >= 3 ){
         // echo $postcustom[2];
          foreach ($postcustom as $key){
            $i = 1;
            if (strpos($key,'oembed_') AND (strpos($key,'oembed_t') == 0) AND ($i == 1) ){
              $video = get_post_custom_values($key);
              $video = preg_match_all('#\bhttps?://[^\s()<>]+(?:\([\w\d]+\)|([^[:punct:]\s]|/))#', $content, $match);
              $string = implode(" ", $match[0]);
              $video= apply_filters('embed_oembed_html', $video[0], $string, '');
        ...
        $content = apply_filters('wp_oembed_get', ($content));
